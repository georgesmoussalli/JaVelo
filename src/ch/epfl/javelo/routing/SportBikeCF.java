...
public record SportBikeCF(Graph graph) implements CostFunction {
    ...
    public double costFactor(int nodeId, int edgeId) {
        ...
        if (edgeAttributes.contains(HIGHWAY_STEPS)) return 55d;
    ...
    private double nonCycleRouteCostFactor(AttributeSet edgeAttributes) {
        ...
        // Main roads
        if (edgeAttributes.contains(HIGHWAY_TRUNK)) return isBike ? 1.2 : 5;
        if (edgeAttributes.contains(HIGHWAY_PRIMARY)) return isBike ? 1.2 : 2;
        ...
        if (edgeAttributes.intersects(TRACKLIKE)) {
            ...
            if (edgeAttributes.contains(TRACKTYPE_GRADE3)) return probablyGood ? 3.0 : 6.0;
            if (edgeAttributes.contains(TRACKTYPE_GRADE4)) return probablyGood ? 4.0 : 8.0;
            if (edgeAttributes.contains(TRACKTYPE_GRADE5)) return probablyGood ? 5.0 : 10.0;
            return probablyGood ? 1.0 : 10.0;
        }
