...
public record VttBikeCF(Graph graph) implements CostFunction{  // Any kind of marked cycle route (international, national, regional or local)
    ...
    public double costFactor(int nodeId, int edgeId) {
        ...
        if (edgeAttributes.contains(HIGHWAY_STEPS)) return 20d;
    ...
    private double nonCycleRouteCostFactor(AttributeSet edgeAttributes) {
        ...
        if (edgeAttributes.intersects(RESIDENTIAL_STREET)) return isUnpaved ? 1.2 : 1.0;
        if (edgeAttributes.contains(HIGHWAY_SERVICE)) return isUnpaved ? 1.3 : 1.1;

        // True iff the edge is designated for bicycle use.
        var isBike = edgeAttributes.intersects(BIKE);

        // Main roads
        if (edgeAttributes.contains(HIGHWAY_TRUNK)) return isBike ? 2.0 : 12;
        if (edgeAttributes.contains(HIGHWAY_PRIMARY)) return isBike ? 1.2 : 3;
        if (edgeAttributes.contains(HIGHWAY_SECONDARY)) return isBike ? 1.5 : 2.1;
        if (edgeAttributes.contains(HIGHWAY_TERTIARY)) return isBike ? 1.2 : 1.6;
        if (edgeAttributes.contains(HIGHWAY_UNCLASSIFIED)) return isBike ? 1.2 : 1.7;
        ...
        if (edgeAttributes.intersects(TRACKLIKE)) {
            ...
            if (edgeAttributes.contains(TRACKTYPE_GRADE2)) return probablyGood ? 1.0 : 1.4;
            if (edgeAttributes.contains(TRACKTYPE_GRADE3)) return probablyGood ? 1.3 : 2.3;
            if (edgeAttributes.contains(TRACKTYPE_GRADE4)) return probablyGood ? 1.7 : 3.0;
            if (edgeAttributes.contains(TRACKTYPE_GRADE5)) return probablyGood ? 2.0 : 4.0;
            return probablyGood ? 1.0 : 3.0;
        }
