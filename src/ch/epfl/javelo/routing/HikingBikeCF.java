...
public record HikingBikeCF(Graph graph) implements CostFunction {
    ...
    public double costFactor(int nodeId, int edgeId) {
        ...
        if (edgeAttributes.contains(HIGHWAY_STEPS)) return 10d;
    ...
    private double nonCycleRouteCostFactor(AttributeSet edgeAttributes) {
        ...
        if (edgeAttributes.intersects(RESIDENTIAL_STREET)) return isUnpaved ? 1.3 : 1.1;
        if (edgeAttributes.contains(HIGHWAY_SERVICE)) return isUnpaved ? 1.4 : 1.2;

        // True iff the edge is designated for bicycle use.
        var isBike = edgeAttributes.intersects(BIKE);

        // Main roads
        if (edgeAttributes.contains(HIGHWAY_TRUNK)) return isBike ? 3.0 : 20;
        if (edgeAttributes.contains(HIGHWAY_PRIMARY)) return isBike ? 2.2 : 3.6;
        if (edgeAttributes.contains(HIGHWAY_SECONDARY)) return isBike ? 2.0 : 3.0;
        ...
        if (edgeAttributes.intersects(TRACKLIKE)) {
            ...
            if (edgeAttributes.contains(TRACKTYPE_GRADE1)) return probablyGood ? 1.0 : 1.1;
            if (edgeAttributes.contains(TRACKTYPE_GRADE2)) return probablyGood ? 1.0 : 1.5;
            if (edgeAttributes.contains(TRACKTYPE_GRADE3)) return probablyGood ? 1.2 : 2.0;
